...
async def transcribe_audio(
    ...
    try:
        ...
        if provider == "openai-whisper" and audio_services.openai_client:
            ...
            result = await transcribe_with_whisper_local(wav_path, language)
        else:
...
async def transcribe_with_whisper_local(audio_path, language):
    """Transcribe using transformers pipeline if available"""
    try:
        from transformers import pipeline  # lazy import
        import torch
        device = 0 if torch.cuda.is_available() else -1
        asr = pipeline("automatic-speech-recognition", model=os.getenv("WHISPER_MODEL", "openai/whisper-base"), device=device)
        ...
        return {
            ...
            "language": language,
            "duration": 0.0,
            "metadata": {"provider": "whisper-local", "model": os.getenv("WHISPER_MODEL", "openai/whisper-base")}
        }
    except Exception as e:
        logger.warning(f"Local whisper transcription unavailable, falling back: {e}")
        return await transcribe_with_speech_recognition(audio_path, language)

@app.post("/adapters/whispercpp")
async def adapter_whispercpp(file: UploadFile = File(...), language: str = Form("en")):
    """REST adapter endpoint to mimic whisper.cpp server as multipart/REST
    Returns {text, confidence, language, duration} JSON"""
    try:
        # Reuse local whisper when available
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            content = await file.read()
            tmp_file.write(content)
            tmp_path = tmp_file.name
        ...
        logger.error(f"Adapter whispercpp failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/adapters/vosk")
async def adapter_vosk(file: UploadFile = File(...), language: str = Form("en")):
    """REST adapter endpoint using Vosk offline ASR.
    Returns {text, confidence, language, duration} JSON"""
    try:
        import wave
        import json
        ...
        model_path = os.getenv("VOSK_MODEL", "model")  # set to a folder with vosk-model
        ...
        if wf.getnchannels() != 1 or wf.getsampwidth() != 2 or wf.getframerate() != 16000:
            raise RuntimeError("Audio must be WAV mono PCM 16k")
        ...
        text = (text + " " + final.get("text", "")).strip()
        wf.close()
        os.unlink(wav_path)
        ...
        return JSONResponse({
            ...
            "language": language,
            "duration": 0.0,
            "metadata": {"provider": "vosk", "model": model_path}
        })
    except Exception as e:
        logger.error(f"Adapter vosk failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
