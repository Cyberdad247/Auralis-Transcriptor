import { http, requestWithRetry } from '../utils/httpClient.js';
...
export class RealTranscriptionService {
    ...
    try {
      ...
      if (process.env.OPENAI_API_KEY) {
        this.openaiClient = new openai.OpenAI({
          ...
          baseURL: process.env.OPENAI_BASE_URL || config.ai.openaiBaseUrl || undefined
        });
        logger.info(`OpenAI client initialized for transcription${(process.env.OPENAI_BASE_URL || config.ai.openaiBaseUrl) ? ' (custom base URL)' : ''}`);
      }
    } catch (error) {
    ...
    try {
      ...
      switch (provider) {
        case 'deepseek':
          ...
          break;
        case 'gemini':
          // Uses AI provider service integration (currently simulated)
          result = await aiProviderService.transcribeWithGemini(audioPath, language);
          break;
        case 'ollama':
          // Route to local whisper pipeline via Python service (transformers-based)
        ...
        case 'google-speech':
          ...
          break;
        case 'whispercpp':
          result = await this.transcribeWithExternalServer(audioPath, config.externalSTT.whisperCppUrl, 'whispercpp', language);
          break;
        case 'vosk':
          result = await this.transcribeWithExternalServer(audioPath, config.externalSTT.voskUrl, 'vosk', language);
          break;
        default:
  ...
  async transcribeWithExternalServer(audioPath, baseUrl, providerName, language) {
    try {
      if (!baseUrl) {
        throw new Error(`${providerName} URL not configured`);
      }
      ...
      const response = await requestWithRetry(() => http.post(baseUrl, formData, {
        headers: {
          ...formData.getHeaders(),
          'Content-Type': 'multipart/form-data'
        },
        timeout: config.transcription.timeout || 300000
      }));
      ...
      logger.error(`${providerName} external transcription failed:`, error.message);
      throw new Error(`${providerName} transcription failed: ${error.message}`);
    }
  }
  ...
  async checkPythonServiceHealth() {
    try {
      const response = await requestWithRetry(() => http.get(`${this.pythonServiceUrl}/health`, {
