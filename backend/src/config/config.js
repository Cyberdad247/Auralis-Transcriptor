...
export const config = {
  ...
  ai: {
    ...
    openaiBaseUrl: process.env.OPENAI_BASE_URL || null // e.g., http://localhost:8080/v1 for LocalAI
  },
  ...
  // External STT server configuration (optional REST adapters)
  externalSTT: {
    whisperCppUrl: process.env.WHISPERCPP_URL || null, // e.g., http://localhost:9000/transcribe (REST adapter)
    voskUrl: process.env.VOSK_URL || null // e.g., http://localhost:2700/transcribe (REST adapter)
  }
};
...
export function validateProviderConfig() {
  ...
  switch (provider) {
    case 'openai-whisper':
      if (!process.env.OPENAI_API_KEY && !(process.env.OPENAI_BASE_URL || config.ai.openaiBaseUrl)) {
        errors.push('OPENAI_API_KEY is required for openai-whisper (or set OPENAI_BASE_URL for LocalAI with any API key)');
      }
      break;
    case 'gemini':
      if (!process.env.GEMINI_API_KEY && !config.ai.geminiApiKey) {
        errors.push('GEMINI_API_KEY is required for gemini provider');
      }
      break;
    case 'deepseek':
      if (!process.env.DEEPSEEK_API_KEY && !config.ai.deepseekApiKey) {
        errors.push('DEEPSEEK_API_KEY is required for deepseek provider');
      }
      break;
    case 'python-service':
      if (!config.pythonService.enabled) {
        errors.push('PYTHON_SERVICE_ENABLED must be true for python-service provider');
      }
      break;
    case 'google-speech':
      if (!config.pythonService.enabled) {
        errors.push('PYTHON_SERVICE_ENABLED must be true for google-speech provider (proxied via python service)');
      }
      break;
    case 'ollama':
      if (!config.pythonService.enabled) {
        errors.push('PYTHON_SERVICE_ENABLED must be true for ollama (whisper-local) provider');
      }
      break;
    case 'whispercpp':
      if (!config.externalSTT.whisperCppUrl) {
        errors.push('WHISPERCPP_URL must be set for whispercpp provider');
      }
      break;
    case 'vosk':
      if (!config.externalSTT.voskUrl) {
        errors.push('VOSK_URL must be set for vosk provider');
      }
      break;
    default:
      // no-op for mock/enhanced/aws-transcribe here
      break;
  }
